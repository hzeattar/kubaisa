...
function ModernLivingHero({ textures }: { textures: ModernTextures }) {
  ...
  return (
    <group position={[-10.6, 0, -11.4]} rotation={[0, Math.PI / 2, 0]}>
      {/* A real room vignette rather than four simultaneous placeholder previews. */}
      ...
      {/* Large neutral rug anchors the set and masks the procedural floor repetition. */}
      ...
      {/* Main low contemporary sofa: broad modules, textured fabric and loose back cushions. */}
      ...
      {/* Sculptural lounge chair makes the vignette read as a curated showroom set. */}
      ...
      {/* Nested marble/metal coffee tables. */}
      ...
      {/* Minimal console and art wall; deliberately editorial, not decorative clutter. */}
...
function ModernPreviewPortal({
  ...
  return (
    <group position={[x, 0, z]} rotation={[0, rotationY, 0]}>
      {/* Closed destination: architecture exists, furniture is intentionally not loaded yet. */}
      <mesh position={[0, 3.25, -0.08]} castShadow receiveShadow>
        <boxGeometry args={[5.7, 6.5, 0.48]} />
        <meshStandardMaterial {...plaster} color="#ddd8d1" roughness={0.72} />
      </mesh>
      <mesh position={[0, 3.0, 0.22]}>
        <boxGeometry args={[4.55, 5.45, 0.1]} />
...
function ModernHallShell({ textures }: { textures: ModernTextures }) {
  ...
  return (
    <group>
      ...
      {[-7.35, 7.35].map((x) => (
        <mesh key={x} position={[x, 4.05, -24]} receiveShadow>
          <boxGeometry args={[0.34, 8.1, 60]} />
      ...
      {/* Wall datum lines create long perspective cues with almost no geometry cost. */}
      {[-7.14, 7.14].map((x) => (
        <mesh key={`datum-${x}`} position={[x, 1.15, -24]}>
          <boxGeometry args={[0.035, 0.09, 58]} />
          <meshStandardMaterial {...metal} color={CHAMPAGNE} metalness={0.78} roughness={0.4} />
        </mesh>
      ))}
...
export function ModernWingHall() {
  ...
  return (
    <group position={[-25, 0, -24]}>
      ...
      {/* Only Living is production-ready. Other room destinations remain light architectural previews. */}
