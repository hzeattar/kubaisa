...
function RoomCamera({room,viewIndex}:{room:InteractiveRoom;viewIndex:number}){
  const controlsRef=useRef<any>(null); const views=ROOM_VIEWS[room]; const view=views[Math.min(viewIndex,views.length-1)];
...
function RoomScene({room,language,onOpenProduct}:{room:InteractiveRoom;language:'ar'|'en';onOpenProduct:(id:string)=>void}){
  ...
  return <><color attach="background" args={[room==='modern'?'#151413':'#17120e']}/><ambientLight intensity={.34} color="#ffe8ca"/><hemisphereLight args={['#fff0da','#1b1510',.58]}/><directionalLight position={[8,13,9]} intensity={1.25} color="#ffe2b6" castShadow shadow-mapSize={[1024,1024]}/>{room==='modern'?<ModernLiving/>:<NeoClassicLiving/>}<PriceHotspot productId={productId} position={room==='modern'?[0,2.25,-4.6]:[0,2.6,-5.2]} language={language} onOpen={onOpenProduct}/></>;
}

export function InteractiveRoomExperience({room,language,onClose}:Props){
  const [viewIndex,setViewIndex]=useState(0); const [selectedProductId,setSelectedProductId]=useState<string|null>(null); const lastWheelAt=useRef(0); const views=ROOM_VIEWS[room];
  const roomTitle=room==='modern'?(language==='ar'?'المعيشة المودرن':'Modern Living'):(language==='ar'?'الصالونات والنيو كلاسيك':'Neo-Classical Salon');
  ...
  return <section className="interactive-room" aria-label={roomTitle} onWheel={handleWheel}>
    ...
    <nav className="room-viewpoints" aria-label={language==='ar'?'نقاط التجول داخل الغرفة':'Room viewpoints'}>{views.map((view,index)=><button type="button" key={`${room}-${index}`} className={index===viewIndex?'is-active':''} onClick={()=>setViewIndex(index)}><span>{String(index+1).padStart(2,'0')}</span>{language==='ar'?view.labelAr:view.labelEn}</button>)}</nav>
